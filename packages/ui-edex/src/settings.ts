...
/** The default theme color — the AIRTRACK steel-cyan accent (analysis.json theme.primaryAccent). */
export const DEFAULT_THEME_COLOR = '#5d9ab8'
...
export const THEME_COLOR_PRESETS: readonly ThemeColorPreset[] = Object.freeze([
  { id: 'analyzed', labelKey: 'edex.preset.analyzed', color: '#5d9ab8' },
...
/**
 * The semantic accents from the AIRTRACK analysis (analysis.json
 * theme.semanticColors — the reference's own altitude-legend hues):
 * warn yellow #FFE500, error red #F00000, info cyan #00C8D8.
 */
...
/**
 * The AIRTRACK chrome constants from analysis.json — this reference's UI is a
 * flat gray/steel console, so the derived family is pinned to the measured
 * values instead of tone()-descending from the accent:
 *   dim     #4A7D96 (theme.secondaryAccent — the lighter steel border tone)
 *   border  #242424 (borderFeatures.cards/dividers — the dark hairline family)
 *   panel2  #1C1C1C (borderFeatures.inputs.fill — the input-surface tone the
 *           bottom widgets' terminal/editor/file surfaces read as)
 */
const AIRTRACK_DIM = '#4a7d96'
const AIRTRACK_BORDER = '#242424'
const AIRTRACK_PANEL2 = '#1c1c1c'
...
/** Derive the full AIRTRACK family from one accent color: the accent itself
 * stays primary; the dim/border/panel2 steps are the reference's own measured
 * chrome constants (see AIRTRACK_* above) rather than a generic darkening. */
export function paletteFor(color: string): EdexPalette {
  const primary = normalizeHex(color) ?? DEFAULT_THEME_COLOR
  return {
    primary,
    dim: AIRTRACK_DIM,
    border: AIRTRACK_BORDER,
    panel2: AIRTRACK_PANEL2,
  }
}
...
export function tokenOverridesFor(palette: EdexPalette): ThemeTokenOverrides {
  // The workspace surface color: the reference's panel/card surface
  // (#313131 — analysis.json theme.panelTone). The workspace must read as the
  // SAME surface as the shell panels (Lessons Learned — never pure black).
  const workspaceSurface = '#313131'
  return {
    // Backgrounds take the panel surface color, not black.
    '--dsw-alias-bg-base': both(workspaceSurface),
    '--dsw-alias-bg-layer-1': both(workspaceSurface),
    '--dsw-alias-bg-layer-2': both(workspaceSurface),
    '--dsw-alias-bg-overlay': both(workspaceSurface),
    ...
    '--dsw-specific-sidebar-fill': both(workspaceSurface),
    '--dsw-specific-input-major': both(workspaceSurface),
  }
}
